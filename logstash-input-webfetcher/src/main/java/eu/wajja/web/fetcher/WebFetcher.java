...
public class WebFetcher implements Input {
	...
	public static final String PROPERTY_EXCLUDE_DATA = "excludeData";
	public static final String PROPERTY_EXCLUDE_LINK = "excludeLink";
	...
	public WebFetcher(String id, Configuration config, Context context) {
		...
		this.excludedDataRegex = config.get(CONFIG_EXCLUDE_DATA).stream().map(url -> (String) url).collect(Collectors.toList());
		this.excludedLinkRegex = config.get(CONFIG_EXCLUDE_LINK).stream().map(url -> (String) url).collect(Collectors.toList());
	...
	public Collection<PluginConfigSpec<?>> configSchema() {
		return Arrays.asList(CONFIG_URLS, CONFIG_DATA_FOLDER, CONFIG_DISABLE_SSL_CHECK, CONFIG_EXCLUDE_DATA, CONFIG_EXCLUDE_LINK, CONFIG_THREAD_POOL_SIZE, CONFIG_REFRESH_INTERVAL, PROXY_HOST, PROXY_PASS, PROXY_PORT, PROXY_USER, CONFIG_MAX_DEPTH, CONFIG_TIMEOUT, CONFIG_MAX_PAGES,
